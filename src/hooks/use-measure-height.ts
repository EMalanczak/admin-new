...
import { useState, useMemo, RefCallback, RefObject, useCallback, useEffect, useRef } from 'react';

// NOTE
// Basically I have no idea what's going on here.
// the source is copied from use-resize-observer npm package
// then I made some changes to make it measuring only the height property
...
function useResolvedElement<T extends Element>(
    ...
    const evaluateSubscription = useCallback(() => {
        ...
        const element: T | null =
            ...
            (refOrElement ? (refOrElement instanceof Element ? refOrElement : refOrElement.current) : null);
...
export function useMeasureHeight<T extends Element>(
    opts: {
                ...
                if (
                    ...
                    resizeObserverRef.current = {
                        ...
                        instance: new ResizeObserver((entries) => {
                            ...
                            if (previous.current !== newHeight) {
                                const newSize = newHeight;
                                previous.current = newHeight;
